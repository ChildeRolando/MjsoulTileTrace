...
import { ManagedMortalRuntime, ManagedMortalRuntimeError, sha256File } from "../src/index.js";
...
async function setup(mode: string) {
  ...
  const runtime = new ManagedMortalRuntime({
    executable: "python", runtimePath, checkpointPath: checkpoint, mortalSourcePath: dir,
    manifest, startTimeoutMs: 2_000, inferenceTimeoutMs: 100,
    environment: { ...process.env, MORTAL_FAKE_MODE: mode },
  });
  const request: LocalMortalInferenceRequest = {
    protocolVersion: LOCAL_MORTAL_PROTOCOL_VERSION, requestId: "request-1", identity: manifest.identity,
  ...
  return { runtime, request, checkpoint, runtimePath };
}
