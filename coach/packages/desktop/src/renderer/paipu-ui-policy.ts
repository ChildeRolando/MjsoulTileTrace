...
export type PaipuImportUiState =
  ...
  | { readonly state: "invalid_self_actor" }
...
// Fixed, human-readable labels only — raw internal error codes are never
// shown to the user.
export function paipuImportStatusLabel(view: PaipuImportUiState): string {
  switch (view.state) {
    case "idle": return "粘贴雀魂牌谱分享链接并选择你的座位。";
    case "invalid_url": return "牌谱链接格式无效";
    case "invalid_self_actor": return "请先选择你的座位（0-3）。";
...
// generic failure — never a raw code passthrough.
export function paipuImportUiStateFromResult(result: {
  ...
  switch (result.status) {
    ...
    case "invalid_self_actor": return { state: "invalid_self_actor" };
