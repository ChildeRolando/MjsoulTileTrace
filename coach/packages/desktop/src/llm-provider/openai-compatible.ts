...
/** Provider-owned transport policy: one initial send and at most one retry. */
export function createOpenAiCoachProvider(input: {
  ...
  return Object.freeze({
    ...
    async complete(request: LlmCoachRequest): Promise<LlmCoachResult> {
      ...
      if (settings === null || !checked.success) {
        return { errorCode: "provider_unavailable", transportRetries: 0 };
      }
      let key: string | null;
      try { key = await input.credentials.readKey(); } catch { key = null; }
      if (key === null) return { errorCode: "provider_unavailable", transportRetries: 0 };
      const attempt = async (transportRetries: 0 | 1): Promise<LlmCoachResult> => {
        ...
        const timeout = new Promise<LlmCoachResult>((resolve) => {
          timer = setTimeout(() => {
            timedOut = true;
            controller.abort();
            resolve({ errorCode: "timeout", transportRetries });
          }, input.timeoutMs ?? 30_000);
        });
        const send = async (): Promise<LlmCoachResult> => {
          try {
            ...
            if (!response.ok) {
              ...
              return { errorCode: response.status === 429 ? "rate_limited" : response.status >= 500 ? "server_error" : "connection_failed", transportRetries };
            }
            let raw: unknown;
            try { raw = await response.json(); } catch (error) {
              if (error instanceof SyntaxError) return { content: "{}", transportRetries };
              throw error;
            }
            ...
            if (typeof content !== "string") return { content: "{}", transportRetries };
            if (content.length === 0 || reflectsProtectedText(content, [key!, checked.data.prompt])) return redactCoachOutput(content, transportRetries);
            const usage = r?.usage;
      ...
      try {
        ...
        if ("content" in first || first.errorCode === "provider_unavailable") return first;
