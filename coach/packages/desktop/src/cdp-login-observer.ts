...
export interface CdpLoginObserver {
  start(): Promise<void>;
  accept(method: string, parameters: unknown): LoginCaptureResult | null;
  close(): void;
}
...
class StatefulCdpLoginObserver implements CdpLoginObserver {
  ...
  readonly #captures = new Map<string, MahjongSoulLoginCapture>();
  #started = false;
  #closed = false;
  ...
  constructor(
    ...
    expected: ExpectedLogin | undefined,
  ) {
    this.#bundle = bundle;
  ...
  accept(method: string, parameters: unknown): LoginCaptureResult | null {
    ...
    try {
      ...
      if (method === "Network.webSocketCreated") {
        ...
        this.#captures.set(requestId, createMahjongSoulLoginCapture({
          bundle: this.#bundle,
...
export function createCdpLoginObserver(input: {
  readonly bundle: MahjongSoulProtocolBundle;
  ...
  try {
    ...
    if (
      ...
      || keys.some((key) => key !== "bundle" && key !== "debuggerPort" && key !== "expected")
    ) {
      throw unsupported();
    }
    const debuggerPort = snapshotDebuggerPort(input.debuggerPort);
    return new StatefulCdpLoginObserver(
      input.bundle,
      debuggerPort,
      input.expected,
    );
