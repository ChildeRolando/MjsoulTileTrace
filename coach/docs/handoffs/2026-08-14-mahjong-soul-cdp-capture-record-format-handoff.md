...
1. **解外层 Wrapper**：`data` 先按 `lq.Wrapper` 解，取 `wrapper.data` 再按 `GameDetailRecords` 解；
2. **解内层 Wrapper + 换名字**：每个 `action.result` 先按 `lq.Wrapper` 解，`name` 是 `.lq.RecordXxx`，`data` 是真正的动作 protobuf；
3. **新 mapper（`Record*` → canonical）**：按 `Record*` 字段布局重写映射（`RecordNewRound`/`RecordDealTile`/`RecordDiscardTile`/`RecordHule` 等），或写一个 `Record*` 适配器接到现有 canonical 事件。

同时把这次抓到的真实字节做成**脱敏 fixture**（RED→GREEN），替换掉原来那个占位 fixture（`official-bundle-frames.json` 里的 `data` 只是 `[1,2,3,4]`），避免以后重踩。
