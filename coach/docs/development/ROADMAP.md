...
凭据仅从主进程 importer 进入；session-key protector 未修改。完整 package 生产交接、
COAC-4 编排及后续 UI 仍未完成；所需全量门禁受环境限制，不能视为 M6-D2 验收完成。
