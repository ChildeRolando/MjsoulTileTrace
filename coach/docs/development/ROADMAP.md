...
本页是当前路线图。旧的完整构想仍可在 [`2026-08-01-llm-riichi-coach-product-roadmap.md`](../superpowers/plans/2026-08-01-llm-riichi-coach-product-roadmap.md) 查阅，但其状态数字和部分缺口已经过时。
...
| M5 雀魂国区接入 | 接近完成 | Electron 登录、加密恢复、最近 30 场、取回、canonical mapper、重放 | 真实牌谱 H1 对照验收；补齐 mapper 待核实语义 |
...
- 用脱敏真实牌谱核对荣和目标、流局原因、杠类型等 mapper 待核实语义。
- 在产品中完成：重启恢复 → 同步目录 → 选择牌谱 → 对照雀魂回放检查重放事件。
