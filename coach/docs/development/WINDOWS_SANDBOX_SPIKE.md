...
日期：2026-09-26。用途：执行独立的离线可用性验证。

按冻结规格 §8 的同日用户批准修订，禁网演练不再是本地模型正确性验收的前置门槛。
本配置保留为可选验证路线；环境启动失败或未运行时只记录“离线可用性未验证”。
真实本地 CPU spike 的资产校验、候选守恒、package 与提交绑定要求见 [VERIFICATION.md](VERIFICATION.md)。
...
后续启动复核：宿主已重启，hypervisor 存在，固件虚拟化开启，`vmcompute`、`HNS`、
`HvHost` 运行；实际启动仍报告 `0x80370106`（虚拟机或容器意外退出）。隔离环境没有生成
runner transcript 或网络/推理回执，故障发生在 runner 执行前，不能归因于产品候选代码。
当前非管理员进程执行 DISM 功能查询返回 740；未进行系统组件重装、服务重启或自动重启。
在宿主启动故障解决前，离线可用性继续保持未验证，不以配置或普通 CPU 推理结果替代；
该环境缺口不阻塞真实本地模型正确性验收。
