...
- 整盘 StructuredAnalysisPackage（M6-C）与 Typed Context Graph substrate（M6-D1）已实现；M6-D2 的 contracts/reasoning baseline（严格 Coach/ReviewReport 契约、grounding/read-back validator、append-only overlay、evidence-only degrade）已实现。COAC-3 已加入主进程单 provider、独立 safeStorage 凭据记录、无 key IPC/preload、冻结 slice prompt 与一次传输重试的窄生成 seam，Electron 组合根已接线。generation 只解析主进程 analysis store 中已验证的 packageId；正常启动从主进程显式 RIICHI_COACH_ANALYSIS_PACKAGE_FILE 加载已有 M6-C package，严格验证后交给同一 store；renderer 不传路径或 package。既有摄取的自动整盘 package 生产与 COAC-4 编排仍待接通。验证仅使用 stubbed HTTP，尚无真实 LLM 验收；review UI、SQLite 会话与跨平台发布仍未实现（M7-A / M7-B / M8）。详见 [COAC-3 交付与变更控制](../plans/2026-09-19-coac-3-provider.md)。
