...
部署用指令源为 `coach/scripts/review-loop/reviewer-instructions.md`，按角色、规则与上下文、
工作流程、裁决门槛、权限边界、输出格式及完成标准组织。修改时同时检查 Controller 的
新工单说明与 v2 spec，避免一处允许参考历史、另一处禁止。部署前备份线上 instructions，
暂停派发并确认 Reviewer/Controller 无活动任务；更新后精确回读，保持模型、推理强度、
权限与历史结果不变，再恢复原运行状态。已保存的旧 job description/hash 不重写。

2026-09-20 用户批准按“仓库律法审查官”的结构重写，但不迁移其写测试/治理文档的权限。
新会话保留；相关历史允许按需取证，不自动注入全部历史、不继承结论。文档路由从
README/CONTEXT 出发；必读 spec 不限制相关文档访问。旧候选内的全面历史禁令由新派发
说明明确替代，仅此上下文条款变化；SHA、rubric、五门、来源校验、轮数及 JSON 不变。

变更控制：范围仅提示词、派发说明、对应规范与测试；没有新抽象或产品包变更。
历史上下文属于参考线索，不成为放行权威，INV-006/007 的版本/来源校验不变。
回归先因旧模板缺少文档路由而失败，更新后 39 项协议/编排/适配器测试通过；
线上指令精确回读一致，disabled 实读扫描 OK、零派发。未进行模型质量 A/B，
这些检查不证明新提示词已提高缺陷发现率；真实效果需由后续评审观察。
