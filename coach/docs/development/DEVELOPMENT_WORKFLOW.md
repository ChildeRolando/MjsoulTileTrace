...
跨多个包或超过一个小提交的改动，在 `coach/docs/specs/` 和 `plans/` 留版本化文档；单点 bug 可以直接用测试描述契约。
...
## 大改动变更控制协议（substantial AI-assisted changes）

超过一个小提交或跨多个包的改动，提交/交接时附一份**简短变更报告**（不必为每个
小提交重复；单点 bug 修复用测试描述契约即可）：
...
- 动了哪些包？每个包为什么必须动？
- 实现细节是否跨包边界？（应走公共 exports 面；有例外必须在报告中点名）
- `npm run check:architecture` 的结果。
...
- 是否创造新的事实/数据/判断来源？
- 它的权威是什么（确定性管线 / 版本化启发式 / 模型 / LLM / 人工验收）？
- 下游输出如何追溯它（`schemaVersion` / `sourceKind` / `gameId` / 内容哈希）？

### Replaceability

- 是否引入对具体 provider/实现（来源、模型、sidecar、服务）的依赖？
- 去掉该依赖是否仍可无连带下游改动地替换？

### Recoverability

- 实现有误时哪条测试/检查应最先失败？
- 该失败能否从仓库 fixture 离线复现？

### Semantic Load

- 引入哪些新的架构级抽象？按下方"抽象准入规则"逐条回答。
- 每个抽象防止什么具体错误、隔离哪条变化轴、为什么最近的既有抽象不是所有者。

## 抽象准入规则（abstraction admission rule）

新架构级抽象（新包、新核心类型、新边界概念）必须回答：

1. 它防止什么具体失败/错误？
2. 它隔离哪条独立变化轴？
3. 为什么最近的既有抽象不是正确所有者？

"更干净的架构""更好的关注点分离""未来可扩展""更可扩展"本身不是充分理由。
判断是否引入抽象前，先按"既有机制 → 小扩展 → 可执行强制"的顺序找现有承载点。
